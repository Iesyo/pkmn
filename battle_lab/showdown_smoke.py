...
NODE_VERSION_PATTERN = re.compile(r"^v?(\d+)")
...
def installed_node_version() -> tuple[int, str]:
    version = run_checked(["node", "--version"], cwd=PROJECT_ROOT).stdout.strip()
    match = NODE_VERSION_PATTERN.match(version)
    if not match:
        raise RuntimeError(f"No se pudo interpretar la versión de Node.js: {version!r}.")
    ...
    *,
    ...
    if install_marker.exists() and (checkout / "node_modules").is_dir():
        with suppress(json.JSONDecodeError, OSError):
            ...
            installed = marker.get("lockHash") == lock_hash and marker.get("nodeMajor") == node_major
    ...
    if build_marker.exists() and (checkout / "dist" / "sim").is_dir():
        with suppress(json.JSONDecodeError, OSError):
            ...
            built = marker.get("commit") == commit and marker.get("nodeMajor") == node_major
    if not built:
        started = time.monotonic()
        run_long_command_with_retries(
            ["npm", "run", "build"],
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if node_major < MINIMUM_NODE_MAJOR:
        raise RuntimeError(
            f"Battle Lab requiere Node.js {MINIMUM_NODE_MAJOR} o posterior para Pokémon Showdown; "
            f"se detectó {node_version}. Actualiza Node.js y vuelve a ejecutar el runner."
        )
