#!/usr/bin/env python3
"""Visible, resumable LIGHT PPO/self-play runner for Battle Lab M-C.

This runner deliberately skips VGC-Bench's expensive in-training evaluation.
The official evaluator runs after training on the isolated M-C holdout, so the
training process can start immediately, emit progress/ETA, and checkpoint often.
"""
...
def run_light(
    *,
    ...
    print("\n🐉 Battle Lab LIGHT M-C", flush=True)
    print("Fase 1/4 · preparando corpus de entrenamiento...", flush=True)
    ...
    print(
        "Fase 3/4 · "
        + (f"reanudando desde {start_step:,} pasos" if resume else "cargando baseline BC M-A/M-B"),
        flush=True,
    )
        ...
        try:
            ...
            class VisibleLightCallback(BaseCallback):
                ...
                def _on_training_start(self) -> None:
                    ...
                    print(
                        f"RL M-C [{'░' * 28}] {start_step:,}/{total_steps:,} "
                        f"({start_step / total_steps:6.1%}) · ETA calculando",
                        flush=True,
                    )
                    write_status(
                        state="training",
            ...
            latest_text.write_text(
                "\n".join(
                    [
                        ...
                        f"SHA256: {result['finalCheckpointSha257']}",
            ...
            current = int(getattr(locals().get("ppo"), "num_timesteps", start_step))
            write_status(
                state="failed",
                currentStep=current,
                progress=min(current / total_steps, 1.0),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Battle Lab visible LIGHT PPO/self-play M-C")
