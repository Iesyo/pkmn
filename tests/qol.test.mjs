...
test("removes demo teams from every runtime team surface and separates team from version selection", async () => {
  ...
  assert.match(dashboard, /<TeamBuilder key=\{builderImport \? `tournament-\$\{builderImport\.token\}` : builderDraft \? `war-room-\$\{builderDraft\.token\}` : builderVersionId\} groups=\{storedGroups\}/);
