          ...
          <div className="mt-2 space-y-2">
            {set.moves.map((move, index) => {
              ...
              const technicalLine = technical && move.name
                ...
                : "";
              return <div key={index} className="rounded-xl border border-white/5 bg-white/[0.015] p-1.5"><div className="grid grid-cols-[minmax(0,1fr)_62px_88px] gap-2"><Combobox items={options} value={move.name || null} onValueChange={(value) => chooseMove(index, value)}><ComboboxInput title={technical?.desc || technical?.shortDesc || undefined} placeholder={`Movimiento ${index + 1}`} className="w-full border-white/10 bg-white/4" showClear /><ComboboxContent className="border-white/10 bg-slate-950"><ComboboxEmpty>No disponible o repetido.</ComboboxEmpty><ComboboxList>{(name: string) => <ComboboxItem key={name} value={name}>{name}</ComboboxItem>}</ComboboxList></ComboboxContent></Combobox><div className="flex items-center justify-center rounded-lg border border-white/7 bg-white/[0.025]">{move.type ? <TypeBadge type={move.type} className="text-[7px]">{move.type.slice(0, 3)}</TypeBadge> : <span className="text-[8px] text-slate-600">Status</span>}</div><InlineDamageRange move={move} outcome={outcome} ready={opponentReady} /></div>{technicalLine ? <div title={technical?.desc || technical?.shortDesc || undefined} className="mt-1 grid gap-0.5 px-1"><p className="font-mono text-[10px] leading-4 text-cyan-100/85">{technicalLine}</p>{technical?.shortDesc ? <p className="line-clamp-1 text-[10px] leading-4 text-slate-400">{technical.shortDesc}</p> : null}</div> : null}</div>;
            })}
          </div>
        </div>
