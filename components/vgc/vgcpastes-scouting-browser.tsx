  ...
  return (
    <div className="space-y-4">
      <section className="overflow-hidden rounded-[28px] border border-white/8 bg-slate-900/45 shadow-[0_32px_90px_rgba(0,0,0,0.25)]">
        ...
        <div className="p-5">
          <div>
            ...
            <p className="mt-1 max-w-3xl text-xs leading-5 text-slate-500">Explora equipos públicos por formato y busca cores de hasta 3 Pokémon. Un resultado debe contener todos los Pokémon seleccionados.</p>
          </div>
              ...
              >
                <ComboboxValue>
                  {(selectedValue: string[]) => (
                    <ComboboxChips
                      ref={pokemonAnchor}
                      aria-label={selectedValue.length ? "Pokémon seleccionados para el core" : undefined}
                      className="min-h-10 border-white/10 bg-slate-950/70"
                    >
                      {selectedValue.map((species) => (
                        <ComboboxChip
                          key={species}
                          aria-label={species}
                          aria-description="Presiona Backspace o Delete para quitarlo"
                          className="bg-cyan-300/10 text-cyan-100"
                        >
                          {species}
                        </ComboboxChip>
                      ))}
                  ...
                  <ComboboxList>
                    {(species: string) => (
                      <ComboboxItem
                        key={species}
                        value={species}
                        disabled={pokemon.length >= MAX_VGCPASTES_POKEMON_FILTERS && !pokemon.includes(species)}
                      >
                        {species}
                      </ComboboxItem>
                    )}
      ...
      <section>
        ...
        {data.teams.length ? <div className="grid gap-3 xl:grid-cols-2">{data.teams.map((team) => <TeamCard key={team.id} team={team} formatLabel={data.format.label} importing={importingTeamId === team.id} importDisabled={Boolean(importingTeamId)} onImport={() => void importTeam(team)} />)}</div> : (
          <div className="rounded-2xl border border-white/7 bg-slate-950/45 px-6 py-16 text-center">
            ...
            <h3 className="mt-3 text-sm font-black text-white">No hay equipos con ese core</h3>
            <p className="mt-1 text-xs text-slate-600">Quita un Pokémon del filtro o prueba otro core del formato.</p>
          </div>
        )}
      ...
      <footer className="rounded-2xl border border-white/7 bg-slate-950/45 px-4 py-3 text-[10px] leading-5 text-slate-600">
        Datos de <a href={data.source.url} target="_blank" rel="noreferrer" className="font-semibold text-cyan-300 hover:text-cyan-200">{data.source.label}</a>. La app pagina y filtra por core en el servidor; un PokéPaste sólo se descarga cuando eliges importarlo al Team Builder.
      </footer>
    </div>
  );
}
