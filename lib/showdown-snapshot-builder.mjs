...
const MOVE_EFFECT_OBJECT_KEYS = ["boosts", "secondary", "secondaries", "self", "condition", "zMove", "maxMove"];
const MOVE_EFFECT_SCALAR_KEYS = [
  "status", "volatileStatus", "drain", "recoil", "multihit", "critRatio", "willCrit", "breaksProtect",
  "hasCrashDamage", "mindBlownRecoil", "struggleRecoil", "basePowerCallback", "damageCallback", "damage",
  "callsMove", "forceSwitch", "selfSwitch", "stealsBoosts", "thawsTarget",
];
const MOVE_TOP_LEVEL_KEYS = new Set([
  "name", "type", "category", "basePower", "accuracy", "pp", "priority", "target", "flags", "desc", "shortDesc",
  ...MOVE_EFFECT_OBJECT_KEYS,
  ...MOVE_EFFECT_SCALAR_KEYS,
]);
...
function normalizeMoveData(raw, override = {}) {
  const effects = {};
  for (const key of MOVE_EFFECT_OBJECT_KEYS) {
    if (raw[key] !== undefined) effects[key] = raw[key];
  }
  for (const key of MOVE_EFFECT_SCALAR_KEYS) {
    if (raw[key] !== undefined) effects[key] = raw[key];
  }

  const accuracy = raw.accuracy === true
    ? true
  ...
  return {
    ...
    basePower: finiteNumber(raw.basePower),
    accuracy,
    pp: finiteNumber(raw.pp),
    priority: finiteNumber(raw.priority),
    target: typeof raw.target === "string" ? raw.target : "normal",
    flags: Object.keys(copyRecord(raw.flags) ?? {}).filter((flag) => Boolean(raw.flags[flag])).sort(),
    desc: typeof raw.desc === "string" ? raw.desc : "",
    shortDesc: typeof raw.shortDesc === "string" ? raw.shortDesc : (typeof raw.desc === "string" ? raw.desc : ""),
    effects,
    details: remainingDetails(raw, MOVE_TOP_LEVEL_KEYS),
...
export async function buildShowdownSnapshot(fetcher = fetch) {
  ...
  for (const id of [...referencedMoveIds].sort()) {
    const raw = moves[id];
    if (!raw) continue;
    moveCatalog[id] = normalizeMoveData(raw, champions.overrideMoveData?.[id] ?? {});
  }
  ...
  const snapshot = {
    metadata: {
      ...
      schema: SHOWDOWN_SNAPSHOT_SCHEMA,
  ...
  snapshot.metadata.regulation = detectChampionsRegulation(snapshot);
  if (snapshot.metadata.regulation === CHAMPIONS_REGULATION) {
