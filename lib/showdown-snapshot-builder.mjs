...
function normalizeAbilityData(raw, fallbackName, override = {}) {
  const merged = { ...(raw ?? {}), ...override };
  return {
    name: typeof merged.name === "string" ? merged.name : fallbackName,
    desc: typeof merged.desc === "string" ? merged.desc : "",
    shortDesc: typeof merged.shortDesc === "string" ? merged.shortDesc : (typeof merged.desc === "string" ? merged.desc : ""),
    rating: typeof merged.rating === "number" ? merged.rating : null,
    num: typeof merged.num === "number" ? merged.num : null,
    details: remainingDetails(merged, ABILITY_TOP_LEVEL_KEYS),
    championsOverride: Object.keys(override).length ? override : undefined,
  };
}
...
function normalizeItemData(raw, override = {}) {
  ...
  return {
    ...
    desc: typeof merged.desc === "string" ? merged.desc : "",
    shortDesc: typeof merged.shortDesc === "string" ? merged.shortDesc : (typeof merged.desc === "string" ? merged.desc : ""),
    details: remainingDetails(merged, ITEM_TOP_LEVEL_KEYS),
    championsOverride: Object.keys(override).length ? override : undefined,
  };
}
...
export async function buildShowdownSnapshot(fetcher = fetch) {
  ...
  for (const id of [...speciesIds].sort()) {
    ...
    const rawAbilities = speciesOverride.abilities ?? raw.abilities ?? {};
    const abilities = [...new Set(Object.values(rawAbilities).filter((ability) => typeof ability === "string"))];
    ...
    species[id] = {
      name: speciesOverride.name ?? raw.name,
      types: speciesOverride.types ?? raw.types,
      baseStats: speciesOverride.baseStats ?? raw.baseStats,
      abilities,
