...
function normalizeMoveData(raw, override = {}) {
  ...
  const effects = {};
  const structuredKeys = ["boosts", "secondary", "secondaries", "self"];
...
function normalizeAbilityData(raw, fallbackName) {
  return {
...
export async function buildShowdownSnapshot(fetcher = fetch) {
  ...
  const itemCatalog = Object.fromEntries(
    [...referencedItemIds]
      ...
      .map((id) => [id, {
        ...
        desc: typeof itemData[id].desc === "string" ? itemData[id].desc : "",
        shortDesc: typeof itemData[id].shortDesc === "string" ? itemData[id].shortDesc : (typeof itemData[id].desc === "string" ? itemData[id].desc : ""),
      }]),
  );
  ...
  for (const id of [...referencedAbilityIds].sort()) {
    abilityCatalog[id] = normalizeAbilityData(abilityData[id], abilityNamesById.get(id) ?? id);
  }
